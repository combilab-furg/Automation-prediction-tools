...
def process_file(driver, row):
    ...
    click_radio_button_by_xpath('/html/body/div[2]/div[2]/div/form/div[3]/div[2]/ul/li[2]/label/input',driver, timeout)
    click_button_by_xpath('/html/body/div[2]/div[2]/div/form/div[6]/button', timeout, driver)
    qmean_value = get_value_by_xpath('/html/body/div[2]/div[3]/div[2]/div[1]/div/span', timeout)
    return float(qmean_value.split(' ')[-1]) if qmean_value else None
...
def main():
	...
	for i, (index, row) in enumerate(filtered_df.iterrows()):
		...
		try:
			...
			qmean_value = process_file(driver, row)
			df.loc[index, "qmean_value"] = qmean_value
